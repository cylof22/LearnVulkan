...
VulkanHardwareVertexBuffer::VulkanHardwareVertexBuffer(const VkPhysicalDevice& rGPU, const VkDevice& device, const void * pVertexData, uint32_t vertexSize, uint32_t stride)
	: m_vertexBuffer(VK_NULL_HANDLE), m_vertexMemory(VK_NULL_HANDLE)
{
	VulkanMemoryBufferMgr::get()->createVertexBuffer(rGPU, device, pVertexData, vertexSize, stride, m_vertexBuffer, m_vertexMemory);
	...
	m_vertexAttributes[0].format = VK_FORMAT_R32G32B32_SFLOAT;
