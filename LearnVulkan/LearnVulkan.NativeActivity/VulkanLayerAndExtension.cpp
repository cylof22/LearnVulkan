...
VkResult VulkanLayerAndExtension::getDeviceExtensionProperties(std::vector<LayerProperties>& deviceLayerProps, VkPhysicalDevice* gpu)
{
	...
	do
	{
		res = vkEnumerateDeviceLayerProperties(*gpu, &instanceLayerCount, nullptr);
		if (res != VK_SUCCESS)
			return res;
		...
		layerProperties.resize(instanceLayerCount);
		res = vkEnumerateDeviceLayerProperties(*gpu, &instanceLayerCount, layerProperties.data());
	} while (res == VK_INCOMPLETE);
	...
	for (auto globalLayerProp : layerProperties)
	{
		...
		res = getExtension(instanceLayerProps, gpu);
		if (res != VK_SUCCESS)
			continue;

		deviceLayerProps.emplace_back(instanceLayerProps);
